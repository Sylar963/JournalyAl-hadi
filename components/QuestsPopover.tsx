...
const QuestItem: React.FC<{ quest: Quest, onToggle: () => void, onDelete: () => void }> = ({ quest, onToggle, onDelete }) => {
    ...
    return (
        <div className="flex items-center space-x-3 group py-2">
            ...
            <button onClick={onDelete} className="text-gray-600 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity" aria-label={t('quests.aria_delete')}>
  ...
  return (
    <div ref={anchorRef} className="absolute top-16 right-4 md:right-6 w-80 bg-gray-900 border border-gray-800 rounded-lg shadow-2xl z-50 animate-popover-enter">
        <div className="p-4 border-b border-gray-800">
            <h3 className="font-semibold text-white">{t('quests.title')}</h3>
            <p className="text-sm text-gray-400">{t('quests.subtitle')}</p>
        </div>
        <div className="p-4 max-h-72 overflow-y-auto">
            {quests.length === 0 ? (
                <p className="text-center text-sm text-gray-500 py-4">{t('quests.empty')}</p>
            ) : (
                <>
        ...
        <div className="p-3 bg-black/25 border-t border-gray-800">
            <form onSubmit={handleAddQuest} className="flex items-center space-x-2">
                <input 
                    ...
                    placeholder={t('quests.add_placeholder')}
                ...
                />
                <button type="submit" disabled={isAdding} className="p-2 bg-yellow-600 text-black rounded-md hover:bg-yellow-700 disabled:opacity-50 transition-colors" aria-label={t('quests.aria_add')}>
